...
class PhotoService:
    ...
    def get_file_path_from_telegram(self, file_id):
        url = f"{self.base_url_telegram}/getFile"
        response = requests.get(url, params={'file_id': file_id})
        return response.json()['result']['file_path']

    def download_photo_from_telegram(self, file_path):
        url = f"{self.file_url_telegram}/{file_path}"
        response = requests.get(url)
        return response.content

    def save_photo(self, file_id, save_path_base64, save_path_image):
        ...
        with open(save_path_base64, 'w') as f:
            f.write(self.photo_base64)
